...
export default async function processYukatelOrder(order: Order): Promise<void> {
  try {
    ...
    console.log("📨 Yukatel Order Request:", JSON.stringify(yukatelOrderRequest, null, 2));
    ...
    const latestOrder = sortedRecentOrders?.[0];

    if (latestOrder) {
      console.log("🔎 Found recent order:", latestOrder.order_id);

      if (latestOrder.editable) {
        console.log("✏️ Order is editable. Updating the order...");
        const updatedOrderResponse: YukatelResponse = await updateYukatelOrder(
          latestOrder.order_id,
          authcode,
          vpnr,
        ...
        if (updatedOrderResponse.status) {
          ...
          console.error("❌ Failed to update the order:", updatedOrderResponse.msg);
        }
      } else {
        console.log("📦 Order not editable. Creating a new order...");
        const response = await createYukatelOrder(authcode, vpnr, yukatelOrderRequest);

        if (response.status) {
          console.log("✅ Order successfully created!");
        } else {
          console.error("❌ Order failed to create:", response.msg);
        }
      }
    } else {
      console.log("📭 No recent order found. Creating a new order...");
